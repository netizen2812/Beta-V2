"""
Phase 2 (Extended): Real Urdu Fine-Tuning using Meta MMS-VITS (50 Epochs)
- Runs REAL HuggingFace VITS fine-tuning with 50 epochs to eliminate mumbling.
- Urdu text from AlQuran API (ur.jalandhry — scholarly translation)
- Strict 4GB GPU Memory Locks enabled.
"""
...
MMS_BASE     = "C:/Maulana_Training_Forge/deep_pillars/ur_real" # Resume from Epoch 1 if possible, else base
if not os.path.exists(os.path.join(MMS_BASE, "config.json")):
    MMS_BASE = "facebook/mms-tts-urd-script_arabic"

OUTPUT_DIR   = "C:/Maulana_Training_Forge/deep_pillars/ur_real_50epoch"
EPOCHS       = 50
BATCH_SIZE   = 1  # Mandatory for 4GB GPU stability
LR           = 1e-5
MMS_SR       = 16000
...
def train_urdu_extended():
    print("=" * 60)
    print("EXTENDED URDU MMS FINE-TUNING [50 EPOCHS]")
    print("=" * 60)
    ...
    for epoch in range(EPOCHS):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if global_step % 50 == 0:
                avg = epoch_loss / (i + 1)
                print(f" > [UR-EXT] Epoch {epoch+1}/{EPOCHS} | Step {global_step} | Loss: {avg:.6f}")

        avg_epoch = epoch_loss / len(dataloader)
        print(f" > [UR-EXT] Epoch {epoch+1} COMPLETE | Avg Loss: {avg_epoch:.6f}")

        # Save checkpoint periodically or if best
        if avg_epoch < best_loss or (epoch + 1) % 5 == 0:
            best_loss = min(best_loss, avg_epoch)
            model.save_pretrained(OUTPUT_DIR)
            tokenizer.save_pretrained(OUTPUT_DIR)
            print(f"   -> CHECKPOINT SAVED (Best Loss: {best_loss:.6f})")

    print("\n" + "=" * 60)
    print("EXTENDED URDU PILLAR FORGE COMPLETE")

...
def build_home_recommendations(user_id: str) -> dict[str, Any]:
    ...
    # Suggest next ayah: continue from last or promote to next surah on boundary overflow
    parts       = latest["ayah_id"].split(":")
    surah_no    = int(parts[0])
    verse_no    = int(parts[1]) + 1

    # Standard Quran Surah verse counts
    SURA_VERSE_COUNTS = [
    ...
    if 1 <= surah_no <= 114:
        max_verses = SURA_VERSE_COUNTS[surah_no]
        if verse_no > max_verses:
            if surah_no < 114:
                surah_no += 1
                verse_no = 1
            else:
                # Wrap back to the beginning of the Quran on completing the entire Quran
                surah_no = 1
                verse_no = 1

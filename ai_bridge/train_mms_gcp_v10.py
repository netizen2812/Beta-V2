"""
GCP CLOUD FORGE v10 — Urdu MMS-VITS Ultra-Hardened
==================================================
- Stability: Pre-downloads the ENTIRE dataset at startup (prevents subprocess SIGSEGV).
- Logic: Zero subprocess calls inside the training loop.
- Precision: FP32 Hardened.
"""
...
parser.add_argument("--output_dir", default="gs://maulana-urdu-forge-project-ffb3710a/ur_mms_v10")
...
LOCAL_WAV_DIR, LOCAL_OUTPUT = "/tmp/urdu_wavs", "/tmp/ur_mms_v10"
...
class UrduDataset(Dataset):
    def __init__(self, csv_path, tokenizer, remote_wav_dir):
        ...
        # 2. MASSIVE PRE-DOWNLOAD (ULTRA STABILITY)
        print(f" > Pre-downloading {len(self.df)} WAVs to prevent loop-time subprocesses...", flush=True)
        os.makedirs(LOCAL_WAV_DIR, exist_ok=True)
        # Use gcloud storage cp -r for maximum speed and stability
        subprocess.run(["gcloud", "storage", "cp", "-r", f"{remote_wav_dir.rstrip('/')}/*", LOCAL_WAV_DIR], check=True)
        
        print(f" > Dataset Hardened: {len(self.df)} pairs cached locally.", flush=True)
...
def train():
    ...
    print(f" > ULTRA-HARDENED: num_workers=0, pre-downloaded, FP32", flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        if avg < best_loss:
            ...
            print(f" > BEST CHECKPOINT SAVED (Loss: {avg:.6f})", flush=True)

...
class UrduGCSDataset(Dataset):
    def __init__(self, csv_path, tokenizer, local_wav_dir):
        ...
        os.system(f"gcloud storage cp {csv_path} {local_csv}")
...
def cloud_forge():
    ...
    os.system(f"gcloud storage cp -r '{args.wav_dir}/*' {LOCAL_WAV_DIR}/")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            os.system(f"gcloud storage cp -r {LOCAL_OUTPUT}/* {args.output_dir}/checkpoint_epoch_{epoch+1}/")

    # Final upload
    print(" > Final upload to GCS...")
    os.system(f"gcloud storage cp -r {LOCAL_OUTPUT}/* {args.output_dir}/final/")
    print("=" * 60)

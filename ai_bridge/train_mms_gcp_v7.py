"""
GCP CLOUD FORGE v7 — Urdu MMS-VITS Performance Optimized
=========================================================
- Hardware-Aware: Auto-calculates batch/acc steps based on VRAM.
- Performance: Mixed Precision (AMP) for 2-3x speedup.
- I/O: Improved lazy-loading with multi-worker prefetching.
- Zero-Dependency: No torchaudio, no rsync SIGSEGV risks.
"""
...
parser.add_argument("--output_dir", default="gs://maulana-urdu-forge-project-ffb3710a/ur_mms_v7")
parser.add_argument("--epochs",     type=int,   default=200)
parser.add_argument("--target_bs",  type=int,   default=16) # Global effective batch size
...
class UrduDataset(Dataset):
    def __init__(self, csv_path, tokenizer, remote_wav_dir):
        ...
        print(f" > Dataset: {len(self.df)} pairs ready for optimized lazy-loading.", flush=True)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if not os.path.exists(wav_path):
                ...
                subprocess.run(["gcloud", "storage", "cp", remote_path, wav_path], capture_output=True, text=True)
            ...
            wav = wav[:MMS_SR * 8] # Cap at 8 seconds for VRAM
...
def train():
    print("=" * 60, flush=True)
    print("MAULANA URDU FORGE v7 [OPTIMIZED / HARDWARE-AWARE]", flush=True)
    print("=" * 60, flush=True)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    # 1. HARDWARE-AWARE BS TUNING
    if device == "cuda":
        vram_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
        print(f" > Detected GPU: {torch.cuda.get_device_name(0)} ({vram_gb:.1f} GB VRAM)")
        
        # Heuristics for VITS on T4 (16GB)
        if vram_gb > 14:
            phys_bs = 4
        elif vram_gb > 7:
            phys_bs = 2
    ...
    print(f" > Tuning: Physical BS: {phys_bs} | Acc Steps: {acc_steps} | Effective BS: {phys_bs * acc_steps}")
    ...
    # Freeze Encoder (Essential for VRAM + Fine-tuning stability)
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            # Telemetry every 25 steps
            if global_step % 25 == 0:
                elapsed = time.time() - start_time
                throughput = (global_step * phys_bs) / elapsed
                vram_used = torch.cuda.memory_reserved(0) / (1024**3) if device == "cuda" else 0
                print(f" > Ep {epoch+1} | Step {global_step} | Loss: {epoch_loss/epoch_batches:.4f} | {throughput:.1f} smp/s | VRAM: {vram_used:.1f}GB", flush=True)
        ...
        print(f" > Epoch {epoch+1} DONE | Avg Loss: {avg:.6f}", flush=True)
    ...
    print("=" * 60)
    print(f"FORGE COMPLETE. Final Best Loss: {best_loss:.6f}")
    print("=" * 60)

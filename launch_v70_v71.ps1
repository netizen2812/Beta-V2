...
function Launch-Job($lang, $version, $zip_path, $output_dir) {
    ...
    # Final Hardened Script: No pip upgrade (avoid breaking pkg_resources), remove torch-xla, force torch versions
    $command = "export USE_XLA=0`n" +
               "export PJRT_DEVICE=CPU`n" +
               "export PYTHONNOUSERSITE=1`n" +
               "pip uninstall -y torch-xla`n" + 
...
# Launch English Job (v70)
Launch-Job "en" "v70" "$BUCKET/en_gcp_forge.zip" "$BUCKET/en_xtts_v70"

# Launch Arabic Job (v71)
Launch-Job "ar" "v71" "$BUCKET/ar_gcp_forge.zip" "$BUCKET/ar_xtts_v71"

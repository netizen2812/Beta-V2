...
headers = {"X-API-Key": os.getenv("INTERNAL_API_KEY", "faith_tech_secret_key_2026")}

try:
    with open(audio_path, "rb") as f:
        files = {"audio_file": (os.path.basename(audio_path), f, "audio/wav")}
        data = {"ayah_id": "1:1", "madhab": "shafi"}
        res = requests.post(f"{AI_BRIDGE_URL}/api/tajweed-check", files=files, data=data, headers=headers, timeout=90)
        print("Status:", res.status_code)
        try:
            print("Response:", json.dumps(res.json(), indent=2))
        except:
...
try:
    ...
    res = requests.post(f"{AI_BRIDGE_URL}/api/maulana-voice", json=body, headers=headers, timeout=60)
    print("Status:", res.status_code)
    try:
